...
const Delta: React.FC = () => {
    ...
    useEffect(() => {
        const loadTags = async () => {
            try {
                ...
                console.log("[HISTORY][DELTA] Loaded tags:", tags);
                setTableData(tags);
            } catch (error) {
    ...
    const fetchChartData = async (): Promise<void> => {
        ...
        try {
            ...
            console.log("[HISTORY][DELTA] Starting fetch with state:", {
                selectedTags,
                startTime,
                endTime,
                interval: selected,
            ...
            console.log("[HISTORY][DELTA] Chart data assigned:", data);
            setChartData(data);
        } catch (error) {
            console.error("Failed to fetch chart data:", error);
            setErrorMessage("Delta history request failed. Check the console for backend response details.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        ...
        >
            <HistoryLayout
                activeTab="delta"
                title={`${t("model.title", "Telemetry Filter")} - Delta`}
                subtitle={t("model.subtitle", "Track absolute value changes traversing time spans.")}
                fetchButtonLabel={t("model.fetchData", "Fetch Delta Data")}
            >
