...
const Aggregated: React.FC = () => {
    ...
    return (
        <div className="space-y-6 pb-12 text-gray-900">
                ...
                {navItems.map((item) => (
                    <button
                        ...
                        className={`py-2 px-4 rounded-md text-sm font-medium transition-all duration-200 
                            ${item.key === "aggregated" ? "bg-white text-blue-600 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}
                    >
                        {item.label}
                    </button>
                ))}
            ...
            <div className="mb-8">
                <h1 className="text-2xl font-semibold text-gray-900 tracking-tight">{t("model.title", "Telemetry Filter")} - Aggregated</h1>
                <p className="text-sm text-gray-500 mt-1">{t("model.subtitle", "Consolidate telemetry streams through interval grouping.")}</p>
            </div>
            ...
            <Button
                ...
                isLoading={loading}
            >
                {loading ? t("model.fetching", "Fetching...") : t("model.fetchData", "Fetch Aggregated Data")}
            </Button>
